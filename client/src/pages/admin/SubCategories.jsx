...
import { getSubCategories, deleteSubCategory } from "../../api/subCategoryApi"; 
...
const SubcategoriesList = () => {
  ...
  useEffect(() => {
    const fetchParents = async () => {
      try {
        const response = await getCategories({ limit: 100 }); // Fetch enough for dropdown
        if (response?.success) {
  ...
  // 2. Fetch SubCategories when filters change
  useEffect(() => {
    fetchSubCategories();
  }, [currentPage, itemsPerPage, selectedParent]); // Removed statusFilter from deps as it's client-side filtered below
  ...
  const fetchSubCategories = async () => {
    try {
      ...
      const params = {
        limit: itemsPerPage,
        page: currentPage,
        categoryId: selectedParent === "all" ? undefined : selectedParent,
      };
  ...
  // Client-side Filtering logic (Search + Status)
  // Note: Parent Category is filtered Server-side for efficiency
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-10 gap-4">
          <div>
            <h1 className="text-3xl font-black text-gray-900 dark:text-white">
              SubCategory <span className="text-[#F47C26]">Management</span>
            </h1>
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
              Organize sub-services under parent categories.
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 p-4 rounded-2xl mb-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="relative group col-span-1 md:col-span-2">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <FaFolder className="text-gray-400 group-focus-within:text-[#F47C26] transition-colors" />
              </div>
              <select
                  ...
                  {filteredSubCategories.length === 0 ? (
                    ...
                    filteredSubCategories.map((sub) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-500/20">
                            {sub.category?.name || "Unknown Parent"}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex items-center justify-end gap-2">
                            <button
                              onClick={() =>
                                navigate(`/admin/subcategories/${sub._id}/edit`)
                              }
